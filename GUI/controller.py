...
component_parameters_frame = tk.Frame(schematic_params, width=280, height=100)

schematic_canvas_frame = tk.Frame(schematic_params, width=700, height=500, background='white')
...
def open_new_window(components):
    ...
    # if TRUE: lift it on top of root window
...
def exitProgram():
    schematic_analysis.destroy()


menu = CTkMenu(schematic_analysis)

schematic_analysis.config(menu=menu)

FONT_SIZE = ("", 10)
fileMenu = CTkMenu(menu)
fileMenu.add_command(label="Open a Schematic", font=FONT_SIZE, command=get_file_path)
fileMenu.add_command(label="Exit", font=FONT_SIZE, command=exitProgram)
menu.add_cascade(label="File", font=FONT_SIZE, menu=fileMenu)

editMenu = CTkMenu(menu)
